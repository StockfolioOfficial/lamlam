...
func (g *gen) newGenMethod(method *ast.Field) *genMethod {
	...
	if len(method.Names) == 0 ||
		!ok ||
		len(methodType.Params.List) > 2 ||
		len(methodType.Results.List) > 2 {
		//TODO: error
		return nil
	}
	...
	res.params = make([]*genValueType, 0, len(methodType.Params.List))
	for _, param := range methodType.Params.List {
		...
		res.params = append(res.params, vt)
	}

	res.results = make([]*genValueType, 0, len(methodType.Results.List))
	for _, result := range methodType.Results.List {
		...
		res.results = append(res.results, vt)
	}
